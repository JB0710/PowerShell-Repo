...
Add-Type -Path "C:\OEM\src\Busylight\BusylightSDK.dll"
...
for ($i=0; $i -le 255; $i += 1) {
    $MyBusyLightColor.RedRgbValue = Get-Random -Maximum $max -Minimum $Min
    $MyBusyLightColor.GreenRgbValue = Get-Random -Maximum $max -Minimum $Min
    $MyBusyLightColor.BlueRgbValue = Get-Random -Maximum $max -Minimum $Min
